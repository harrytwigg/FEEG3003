...
\framebox[1.1\width]{
    \begin{minipage}{0.8\textwidth}
        Billinear interpolation is a mathematical technique enabling the estimation of new values between existing discrete values that are the functions of two variables. In DDSP it is used to increase the sample rate of that of the neural network to that of the underlying audio signal.
    \end{minipage}
}
\vspace{0.5cm}
...
As all of these quantities are time dependant, they can be denoted as such: $f(t), l(t), z(t)$.
...
\framebox[1.1\width]{
    \begin{minipage}{0.8\textwidth}
        CREPE is a highly accurate pitch detection based \acrfull{CNN}\cite{CREPE}. It can acturately predict pitch with an accuracy of over 99.9\%. It is a pre-trained model that can be used with the DDSP library to accurately extract the pitch from the input signal.
    \end{minipage}
}
\vspace{0.5cm}
...
\framebox[1.1\width]{
    \begin{minipage}{0.8\textwidth}
        L1 loss is defined as the sum of the absolute differences\cite{L1Statistic}, in this case the sum of the absolute differences between the magnitude spectrograms.
    \end{minipage}
}
\vspace{0.5cm}
...
The variation of DDSP designed for \nameref{sec:singing_voice_synthesis} was chosen as this is a relatively novel area of research and held the most exciting applicability. However this has historically proven to be a difficult problem to solve. This is because the human voice is not a simple harmonic series, with a constant timbre.